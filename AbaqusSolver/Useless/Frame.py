...
class uFrame(object):
    ...
    def __constructVerticesSystem(self):
        ...
        for b in self.beams:
            ...
            #
            jNodes[j] = y
            # list(self.__getConnectedComponents(i-1,j))
            # list(self.__getConnectedComponents(i,j))
            vertices[i-1][j] = uVerticeFrame(x=x1,y=y,index=(i-1,j),connectedComponents=None) # 梁构件左侧节点
            vertices[i][j] = uVerticeFrame(x=x2,y=y,index=(i,j),connectedComponents=None)   # 梁构件右侧节点
