...
def setBoundary(assembly,instance,frame,load,):
    ...
    '''
    # 创建对应的 node set
    bottomRg = frame.getBottomSlabRegion()
    ...
    # 创建弹簧边界
    if load.kx>0:   # 看情况考虑是否设置水平弹簧
        assembly.engineeringFeatures.SpringDashpotToGround(
                name='bottomSpring1', region=s1, orientation=None, dof=1,
                springBehavior=True, springStiffness=load.kx, dashpotBehavior=False,
                dashpotCoefficient=0.0)

    assembly.engineeringFeatures.SpringDashpotToGround(
            name='bottomSpring2', region=s1, orientation=None, dof=2,
            springBehavior=True, springStiffness=load.ky, dashpotBehavior=False,
            dashpotCoefficient=0.0)

def setConcentratedForce(model,instance,stepName,frame,load,):
    '''
    :param model:
    :param instance:
    ...
    :type frame: uFrame
    '''
    for iN in range(0,frame.iNodes.__len__()):
        for jN in range(0,frame.jNodes.__len__()):
            x,y = frame.iNodes[iN] , frame.jNodes[jN] # 节点的二维坐标
            weight = frame.getNodeWeight(iNodeIndex= iN,jNodeIndex= jN) # 与节点相连的所有构件的重量之各的一半
            ...
            force = -1 * weight * load.kc  # 施加的集中力的值
...
def setLineLoad(model, instance, stepName, frame, load):
    ...
    yt, yb = frame.jNodes[-1] , frame.jNodes[0] # 分别对应框架的顶部与底部标高
    ...
    model.LineLoad(name=loadName, createStepName=stepName, region=region, comp1=load.pMax , distributionType=FIELD, field=expreName)
